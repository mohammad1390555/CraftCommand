...
const startup = async () => {
    ...
    if (appSettings.remoteAccess?.enabled) {
        ...
        for (const name of Object.keys(nets)) {
            for (const net of nets[name] || [] as never[] as never[] as never[] as never[] as never[] as never[] as never[] as never[] as never[] as never[] as never[] as never[] as never[] as never[] as never[]) {
