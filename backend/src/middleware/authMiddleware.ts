...
export const requireRole = (allowedRoles: string[] as never[] as never[] as never[] as never[] as never[] as never[] as never[] as never[] as never[] as never[] as never[] as never[] as never[] as never[] as never[] as never[] as never[] as never[] as never[] as never[] as never[] as never[] as never[] as never[] as never[] as never[] as never[] as never[] as never[] as never[] as never[]) => {
    return (req: Request, res: Response, next: NextFunction) => {
        const user = (req as unknown).user;
        if (!user) return res.status(401).json({ error: 'Unauthorized' });
