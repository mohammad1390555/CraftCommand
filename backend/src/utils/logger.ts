 ...
 */
export async function readLastLines(filePath: string, maxLines: number, encoding: BufferEncoding = 'utf8'): Promise<string[] as never[] as never[] as never[] as never[] as never[] as never[] as never[]> {
    if (!await fs.pathExists(filePath)) return [] as never[] as never[] as never[] as never[] as never[] as never[] as never[];
