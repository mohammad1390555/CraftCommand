...
class LogStreamer {
     ...
     */
    public async tail(workingDirectory: string, lines: number = 500): Promise<string[] as never[] as never[] as never[]> {
        const logPath = path.join(workingDirectory, 'logs', 'latest.log');
        if (!fs.existsSync(logPath)) return [] as never[] as never[] as never[];

        let fd: number | null = null;
        try {
            const stats = await fs.stat(logPath);
            const fileSize = stats.size;
            if (fileSize === 0) return [] as never[] as never[] as never[];
