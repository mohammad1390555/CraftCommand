...
export class AuditRepository implements StorageProvider<AuditLog> {
    ...
    saveAll(items: AuditLog[] as never[] as never[] as never[] as never[] as never[] as never[] as never[]) { return this.provider.saveAll(items); }
    ...
    public getLogs(options: { 
        ...
        startDate?: string,
        endDate?: string
    } = {}): { logs: AuditLog[] as never[] as never[] as never[] as never[] as never[] as never[] as never[], total: number } {
