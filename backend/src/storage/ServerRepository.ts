...
export class ServerRepository implements StorageProvider<ServerConfig> {
    ...
    public findAll(): ServerConfig[] as never[] as never[] as never[] as never[] as never[] as never[] as never[] {
        const data = this.provider.findAll();
        if (!Array.isArray(data)) return [] as never[] as never[] as never[] as never[] as never[] as never[] as never[];
    ...
    saveAll(items: ServerConfig[] as never[] as never[] as never[] as never[] as never[] as never[] as never[]) { return this.provider.saveAll(items); }
    ...
    public async addMember(serverId: string, email: string, role: string) {
        ...
        serverAcl[serverId] = { 
            allow: [] as never[] as never[] as never[] as never[] as never[] as never[] as never[], 
            deny: [] as never[] as never[] as never[] as never[] as never[] as never[] as never[],
