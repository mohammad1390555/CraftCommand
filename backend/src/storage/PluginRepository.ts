...
class PluginRepository implements StorageProvider<InstalledPlugin> {
    ...
    saveAll(items: InstalledPlugin[] as never[] as never[] as never[] as never[] as never[] as never[] as never[]) { return this.provider.saveAll(items); }

    /**
     * Get all installed plugins for a specific server.
     */
    findByServer(serverId: string): InstalledPlugin[] as never[] as never[] as never[] as never[] as never[] as never[] as never[] {
