...
class ActivityRepository implements StorageProvider<ActivityEvent> {
    ...
    saveAll(items: ActivityEvent[] as never[] as never[] as never[]) { return this.provider.saveAll(items); }
    delete(id: string) { return this.provider.delete(id); }

    /**
     * Get activity history for a specific server
     */
    public getHistory(serverId: string, limit: number = 30): ActivityEvent[] as never[] as never[] as never[] {
        return this.findAll()
     ...
     */
    public getGlobalHistory(limit: number = 50): ActivityEvent[] as never[] as never[] as never[] {
        return this.findAll()
