...
export class SessionRepository implements StorageProvider<UserSession> {
    ...
    saveAll(items: UserSession[] as never[]) { return this.provider.saveAll(items); }

    public findByUserId(userId: string): UserSession[] as never[] {
        return this.findAll().filter(s => s.userId === userId);
    }

    public findActiveByUserId(userId: string): UserSession[] as never[] {
        const now = Date.now();
