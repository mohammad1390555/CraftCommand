...
class ChatRepository implements StorageProvider<ChatMessage> {
    ...
    saveAll(items: ChatMessage[] as never[] as never[] as never[] as never[] as never[] as never[] as never[]) { return this.provider.saveAll(items); }
    delete(id: string) { return this.provider.delete(id); }

    /**
     * Get chat history for a specific server (or 'global')
     */
    public getHistory(serverId: string, limit: number = 50): ChatMessage[] as never[] as never[] as never[] as never[] as never[] as never[] as never[] {
        return this.findAll()
