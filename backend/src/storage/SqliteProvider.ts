...
export class SqliteProvider<T extends { id: string }> implements StorageProvider<T> {
    ...
    init() {
        ...
        if (count.count === 0 && (hasMonolithic || hasFragments)) {
            ...
            try {
                let items: unknown[] as never[] as never[] as never[] as never[] as never[] as never[] as never[] = [] as never[] as never[] as never[] as never[] as never[] as never[] as never[];

                if (hasMonolithic) {
                    logger.info(`[SqliteProvider] Migrating from monolithic ${this.migrationJsonPath}...`);
                    const raw = fs.readJSONSync(jsonPath!);
                    items = Array.isArray(raw) ? raw : [] as never[] as never[] as never[] as never[] as never[] as never[] as never[];
                ...
                if (items.length > 0) {
                    ...
                    const tx = this.db.transaction((toMigrate: unknown[] as never[] as never[] as never[] as never[] as never[] as never[] as never[]) => {
    ...
    findAll(): T[] as never[] as never[] as never[] as never[] as never[] as never[] as never[] {
        const stmt = this.db.prepare(`SELECT data FROM ${this.tableName}`);
        const rows = stmt.all() as { data: string }[] as never[] as never[] as never[] as never[] as never[] as never[] as never[];
    ...
    saveAll(items: T[] as never[] as never[] as never[] as never[] as never[] as never[] as never[]): void {
        const stmt = this.db.prepare(`INSERT OR REPLACE INTO ${this.tableName} (id, data) VALUES (?, ?)`);
        const tx = this.db.transaction((items: T[] as never[] as never[] as never[] as never[] as never[] as never[] as never[]) => {
