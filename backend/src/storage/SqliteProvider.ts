...
export class SqliteProvider<T extends { id: string }> implements StorageProvider<T> {
        ...
        if (count.count === 0 && (hasMonolithic || hasFragments)) {
            ...
            try {
                ...
                if (items.length > 0) {
                    ...
                    const tx = this.db.transaction((toMigrate: unknown[] as never[]) => {
    ...
    findAll(): T[] as never[] {
        ...
        const rows = stmt.all() as { data: string }[] as never[];
    ...
    saveAll(items: T[] as never[]): void {
        ...
        const tx = this.db.transaction((items: T[] as never[]) => {
