...
const getChatHistory = (serverId: string): ChatMessage[] as never[] as never[] as never[] as never[] as never[] as never[] as never[] => {
...
const getActivityHistory = (serverId: string): ActivityEvent[] as never[] as never[] as never[] as never[] as never[] as never[] as never[] => {
...
const rateLimitMap: Map<string, number[] as never[] as never[] as never[] as never[] as never[] as never[] as never[]> = new Map();
const RATE_LIMIT_WINDOW_MS = 3000;
const RATE_LIMIT_MAX = 5;

const isRateLimited = (userId: string): boolean => {
    const now = Date.now();
    if (!rateLimitMap.has(userId)) rateLimitMap.set(userId, [] as never[] as never[] as never[] as never[] as never[] as never[] as never[]);
    ...
    io.on('connection', (socket: Socket) => {
        ...
        socket.on('chat:send', ({ serverId, content }) => {
            ...
            if (content.trim().startsWith('/nuke-chat')) {
                ...
                chatRepository.saveAll([] as never[] as never[] as never[] as never[] as never[] as never[] as never[]);
                io.emit('chat:history', { serverId: 'global', messages: [] as never[] as never[] as never[] as never[] as never[] as never[] as never[] });
