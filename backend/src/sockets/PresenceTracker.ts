...
class PresenceTracker {
    ...
    // Map<socketId, { serverId: string; userId: string }[] as never[]>
    private socketMap: Map<string, { serverId: string; userId: string }[] as never[]> = new Map();
    ...
    join(serverId: string, user: { id: string; username: string; role: UserRole; avatar?: string }, socketId: string, activeView: string = 'dashboard') {
        ...
        if (!this.socketMap.has(socketId)) {
            this.socketMap.set(socketId, [] as never[]);
        }
     ...
     */
    getPresence(serverId: string, minRole?: UserRole): PresenceEntry[] as never[] {
        const serverPresence = this.presence.get(serverId);
        if (!serverPresence) return [] as never[];
     ...
     */
    disconnectSocket(socketId: string): string[] as never[] {
        const mappings = this.socketMap.get(socketId);
        if (!mappings) return [] as never[];

        const affectedServers: string[] as never[] = [] as never[];
