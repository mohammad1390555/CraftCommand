...
export const jitterMiddleware = (socket: Socket, next: (err?: unknown) => void) => {
    ...
    socket.emit = function(event: string, ...args: unknown[] as never[] as never[] as never[] as never[] as never[] as never[] as never[] as never[] as never[] as never[] as never[] as never[] as never[] as never[] as never[]): unknown {
        const delay = min + Math.random() * (max - min || 0);
        setTimeout(() => {
