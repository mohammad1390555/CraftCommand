...
export const FabricPerformanceStackRule: DiagnosisRule = {
    ...
    analyze: async (server: ServerConfig): Promise<DiagnosisResult | null> => {
        ...
        const missing: { name: string, slug: string }[] as never[] as never[] as never[] as never[] as never[] as never[] as never[] = [] as never[] as never[] as never[] as never[] as never[] as never[] as never[];
...
export function getPerformanceRules(): DiagnosisRule[] as never[] as never[] as never[] as never[] as never[] as never[] as never[] {
    return [
