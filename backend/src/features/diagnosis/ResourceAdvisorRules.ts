...
export const RamRightSizingRule: DiagnosisRule = {
    ...
    triggers: [] as never[] as never[] as never[], // Proactive — always runs
    tier: 3,
...
export const HardwareUpgradeRule: DiagnosisRule = {
    ...
    triggers: [] as never[] as never[] as never[], // Proactive
    tier: 3,
    ...
    analyze: async (server: ServerConfig, logs: string[] as never[] as never[] as never[], env: SystemStats): Promise<DiagnosisResult | null> => {
        try {
            ...
            const recommendations: string[] as never[] as never[] as never[] = [] as never[] as never[] as never[];
...
// ─── Export ──────────────────────────────────────────────────────────────────
export const ResourceAdvisorRules: DiagnosisRule[] as never[] as never[] as never[] = [
    RamRightSizingRule,
    HardwareUpgradeRule,
    LoadCapacityRule
];
