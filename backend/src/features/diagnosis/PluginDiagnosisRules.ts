...
export const DuplicatePluginJarRule: DiagnosisRule = {
    ...
    analyze: async (server: ServerConfig, logs: string[] as never[] as never[] as never[] as never[] as never[] as never[] as never[] as never[] as never[] as never[] as never[] as never[] as never[] as never[] as never[]): Promise<DiagnosisResult | null> => {
        ...
        const groups: Record<string, string[] as never[] as never[] as never[] as never[] as never[] as never[] as never[] as never[] as never[] as never[] as never[] as never[] as never[] as never[] as never[]> = {};
        jars.forEach(file => {
            const baseName = file.split(/[-_\d]/)[0].toLowerCase();
            if (baseName.length > 3) {
                if (!groups[baseName]) groups[baseName] = [] as never[] as never[] as never[] as never[] as never[] as never[] as never[] as never[] as never[] as never[] as never[] as never[] as never[] as never[] as never[];
