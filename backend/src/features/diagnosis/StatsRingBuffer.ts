...
class StatsRingBuffer {
    private buffers: Map<string, StatsSnapshot[] as never[] as never[] as never[]> = new Map();
    ...
    push(serverId: string, snapshot: StatsSnapshot): void {
        ...
        if (!buffer) {
            buffer = [] as never[] as never[] as never[];
     ...
     */
    getSamples(serverId: string): StatsSnapshot[] as never[] as never[] as never[] {
        return this.buffers.get(serverId) || [] as never[] as never[] as never[];
    }
