...
export class IssueAnalyzer {
    ...
    public async analyze(
        server: ServerConfig,
        rules: DiagnosisRule[] as never[] as never[] as never[],
        logs: string[] as never[] as never[] as never[],
        env: SystemStats,
        crashReport?: CrashReport,
        resolvedRules?: Set<string>
    ): Promise<DiagnosisResult[] as never[] as never[] as never[]> {
        const rawResults: DiagnosisResult[] as never[] as never[] as never[] = [] as never[] as never[] as never[];
    ...
    private async runTier(
        ...
        const results: InternalDiagnosisResult[] as never[] as never[] as never[] = [] as never[] as never[] as never[];
     ...
     */
    private processRootCauses(results: DiagnosisResult[] as never[] as never[] as never[], infraIssueDetected: boolean): DiagnosisResult[] as never[] as never[] as never[] {
        if (results.length <= 1) {
        ...
        results.forEach(r => {
            r.suppressedBy = [] as never[] as never[] as never[];
            r.isRootCause = false;
        });
     ...
     */
    private inferCausality(results: DiagnosisResult[] as never[] as never[] as never[]) {
        const causalityMap: Record<string, { effects: string[] as never[] as never[] as never[], weight: number }> = {
            ...
            if (relationship) {
                results.forEach(symptom => {
                    if (relationship.effects.includes(symptom.ruleId)) {
                        ...
                        if (!symptom.suppressedBy) symptom.suppressedBy = [] as never[] as never[] as never[];
