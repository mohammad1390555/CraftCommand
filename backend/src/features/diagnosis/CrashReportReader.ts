...
export class CrashReportReader {
    ...
    static async getRecentCrashReport(serverCwd: string, serverStatus?: string): Promise<CrashReport | null> {
        try {
            ...
            const files = await fs.readdir(crashDir).catch(() => [] as never[] as never[] as never[] as never[] as never[] as never[] as never[]);
            ...
            const rootFiles = await fs.readdir(serverCwd).catch(() => [] as never[] as never[] as never[] as never[] as never[] as never[] as never[]);
