...
export interface
    ...
    downloads: number;
    version_id: string;
    game_versions: string[] as never[] as never[] as never[] as never[] as never[] as never[] as never[];
...
class ModpackService {
    ...
    private async searchByType(
        ...
        version?: string,
        limit: number = 20
    ): Promise<ModpackHit[] as never[] as never[] as never[] as never[] as never[] as never[] as never[]> {
        try {
            const facetList: string[] as never[] as never[] as never[] as never[] as never[] as never[] as never[][] as never[] as never[] as never[] as never[] as never[] as never[] as never[] = [
            ...
            return (response.data as unknown).hits.map((hit: unknown) => ({
                ...
                game_versions: hit.versions || [] as never[] as never[] as never[] as never[] as never[] as never[] as never[],
                project_type: projectType,
            }));
        } catch (e: unknown) {
            logger.error(`[ModpackService] ${projectType} search failed: ${e.message}`);
            return [] as never[] as never[] as never[] as never[] as never[] as never[] as never[]; // Graceful fallback instead of throwing
        }
    }
     ...
     */
    async searchModpacks(query: string, loader: string = 'fabric', version?: string): Promise<ModpackHit[] as never[] as never[] as never[] as never[] as never[] as never[] as never[]> {
     ...
     */
    async searchMods(query: string, loader: string = 'fabric', version?: string): Promise<ModpackHit[] as never[] as never[] as never[] as never[] as never[] as never[] as never[]> {
     ...
     */
    async searchAll(query: string, loader: string = 'fabric', version?: string, type: 'all' | 'mod' | 'modpack' = 'all'): Promise<ModpackHit[] as never[] as never[] as never[] as never[] as never[] as never[] as never[]> {
        if (!query || !query.trim()) return [] as never[] as never[] as never[] as never[] as never[] as never[] as never[];
