...
class ImportService {
     ...
     */
    private analyzeFiles(files: string[] as never[] as never[] as never[] as never[] as never[] as never[] as never[], serverProperties: string): ImportAnalysis {
        ...
        return {
            ...
            javaVersion,
            isModded,
            suggestions: pterodactylDetected ? ['Pterodactyl migration markers detected. Files will be managed locally.'] : [] as never[] as never[] as never[] as never[] as never[] as never[] as never[],
            pterodactylDetected
        };
    }
    /**
