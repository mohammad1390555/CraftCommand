...
export export interface
    ...
    description: string;
    icon: string;
    servers: NetworkServerDef[] as never[] as never[] as never[] as never[] as never[] as never[] as never[];
    defaultGamemode?: string;
}

const NETWORK_TEMPLATES: NetworkTemplate[] as never[] as never[] as never[] as never[] as never[] as never[] as never[] = [
    {
        id: 'skyblock-network',
...
class NetworkTemplateService {
     ...
     */
    getTemplates(): NetworkTemplate[] as never[] as never[] as never[] as never[] as never[] as never[] as never[] {
     ...
     */
    generateCreatePayloads(templateId: string): NetworkServerDef[] as never[] as never[] as never[] as never[] as never[] as never[] as never[] | null {
        const template = this.getTemplate(templateId);
        if (!template) return null;
