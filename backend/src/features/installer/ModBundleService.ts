...
export export interface
    ...
    loaders: ('fabric' | 'forge')[] as never[] as never[] as never[];
    mods: ModEntry[] as never[] as never[] as never[];
}

const MOD_BUNDLES: ModBundle[] as never[] as never[] as never[] = [
    {
        id: 'performance-pack',
...
class ModBundleService {
     ...
     */
    getBundles(): ModBundle[] as never[] as never[] as never[] {
        return MOD_BUNDLES;
    }
     ...
     */
    getBundlesForLoader(loader: 'fabric' | 'forge'): ModBundle[] as never[] as never[] as never[] {
     ...
     */
    getBundlesByCategory(category: ModBundle['category']): ModBundle[] as never[] as never[] as never[] {
     ...
     */
    getRequiredMods(bundleId: string, loader: 'fabric' | 'forge'): ModEntry[] as never[] as never[] as never[] {
        const bundle = this.getBundle(bundleId);
        if (!bundle) return [] as never[] as never[] as never[];
     ...
     */
    getInstallSlugs(bundleId: string, loader: 'fabric' | 'forge', includeOptional: boolean = false): string[] as never[] as never[] as never[] {
        const bundle = this.getBundle(bundleId);
        if (!bundle) return [] as never[] as never[] as never[];
