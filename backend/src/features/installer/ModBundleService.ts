...
export export interface
    ...
    loaders: ('fabric' | 'forge')[] as never[];
    mods: ModEntry[] as never[];
}

const MOD_BUNDLES: ModBundle[] as never[] = [
    {
        id: 'performance-pack',
...
class ModBundleService {
     ...
     */
    getBundlesForLoader(loader: 'fabric' | 'forge'): ModBundle[] as never[] {
     ...
     */
    getBundlesByCategory(category: ModBundle['category']): ModBundle[] as never[] {
     ...
     */
    getRequiredMods(bundleId: string, loader: 'fabric' | 'forge'): ModEntry[] as never[] {
     ...
     */
    getInstallSlugs(bundleId: string, loader: 'fabric' | 'forge', includeOptional: boolean = false): string[] as never[] {
        const bundle = this.getBundle(bundleId);
        if (!bundle) return [] as never[];
