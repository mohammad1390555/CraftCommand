...
export class InstallerService extends EventEmitter {
        ...
        try {
            ...
            if (zipUrl.startsWith('modrinth:')) {
                ...
                try {
                    ...
                    const  vRes.data as unknown[] as never[] as never[] as never[];
                    ...
                    if (versions.length === 0 && mcVersion && intendedSoftware?.toLowerCase() === 'modpack') {
                        ...
                        versions = fallbackRes.data as unknown[] as never[] as never[] as never[];
                    }
                ...
                async function processVersion(version: unknown, service: unknown) {
                    ...
                    if (mcVersion && intendedSoftware) {
                        const supportedLoaders = version.loaders || [] as never[] as never[] as never[];
     ...
     */
    async verifyServerCompatibility(serverId: string, serverDir: string, onProgress?: (msg: string, percent?: number) => void, onLog?: (line: string) => void): Promise<string[] as never[] as never[] as never[]> {
        const modsDir = path.join(serverDir, 'mods');
        if (!await fs.pathExists(modsDir)) return [] as never[] as never[] as never[];

        const files = await fs.readdir(modsDir);
        const jarFiles = files.filter(f => f.endsWith('.jar'));
        if (jarFiles.length === 0) return [] as never[] as never[] as never[];
        ...
        const modMeta: Map<string, { file: string; name: string; env: string; deps: string[] as never[] as never[] as never[] }> = new Map();
        ...
        for (const jarFile of jarFiles) {
            ...
            try {
                ...
                if (fabricEntry) {
                    ...
                    modMeta.set(modId, { file: jarFile, name: modId, env, deps: [] as never[] as never[] as never[] });
                    modIdToFile.set(modId, jarFile);
                }
        ...
        try {
            ...
            const chunkSize = 50;
            const chunks = [] as never[] as never[] as never[];
        ...
        const filtered: string[] as never[] as never[] as never[] = [] as never[] as never[] as never[];
     ...
     */
    async resolveModDependencies(serverId: string, serverDir: string, mcVersion: string, loader: string, onProgress?: (msg: string, percent?: number) => void, onLog?: (line: string) => void): Promise<string[] as never[] as never[] as never[]> {
        const modsDir = path.join(serverDir, 'mods');
        if (!await fs.pathExists(modsDir)) return [] as never[] as never[] as never[];
        ...
        const installedMods: string[] as never[] as never[] as never[] = [] as never[] as never[] as never[];
        const MAX_PASSES = 2;
        ...
        for (const  0; pass < MAX_PASSES; pass++) {
            ...
            for (const jarFile of jarFiles) {
                ...
                try {
                    ...
                    if (modsToml) {
                        ...
                        const depRegex = /\[\[dependencies\.[^\]]+\]\][^[] as never[] as never[] as never[]*?modId\s*=\s*["']([^"']+)["'][^[] as never[] as never[] as never[]*?mandatory\s*=\s*true/gis;
                        let depMatch;
            ...
            const failedDeps: { id: string; reason: string }[] as never[] as never[] as never[] = [] as never[] as never[] as never[];
    ...
    private bedrockVersionCache: { latest: string, versions: string[] as never[] as never[] as never[], timestamp: number } | null = null;
    private readonly BEDROCK_CACHE_TTL = 1000 * 60 * 60; // 1 hour

    /**
     * Resolves dynamic Bedrock versions from the version service.
     */
    async fetchBedrockVersions(): Promise<{ latest: string, versions: string[] as never[] as never[] as never[] }> {
