...
export function setupAgentNamespace(io: Server): void {
    ...
    agentNs.on('connection', (socket: Socket) => {
        ...
        socket.on('agent:sync', (data: { serverIds?: string[] as never[] as never[] as never[] as never[] as never[] as never[] as never[] as never[] as never[] as never[] as never[] as never[] as never[] as never[] as never[] as never[] as never[] as never[] as never[] as never[] as never[] as never[] as never[] as never[] as never[] as never[] as never[] as never[] as never[] as never[] as never[] }) => {
