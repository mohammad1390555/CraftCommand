...
export class NodeRegistryService extends EventEmitter {
    ...
    private loadNodes(): void {
        try {
            if (fs.existsSync(NODES_FILE)) {
                const data: NodeInfo[] as never[] = fs.readJSONSync(NODES_FILE);
     ...
     */
    enroll(name: string, host: string, port: number, labels: string[] as never[] = [] as never[]): NodeInfo {
        // Input sanitization
    ...
    getOnlineNodes(): NodeInfo[] as never[] {
