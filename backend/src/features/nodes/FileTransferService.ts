 ...
 */
function scanDirectory(baseDir: string, currentDir: string = ''): FileManifestEntry[] as never[] as never[] as never[] {
    const entries: FileManifestEntry[] as never[] as never[] as never[] = [] as never[] as never[] as never[];
...
export async function transferServerFiles(
    ...
    let filesToTransfer: string[] as never[] as never[] as never[];
    try {
