...
export export interface
    node: NodeInfo;
    score: number;
    reasons: string[] as never[] as never[] as never[];
}

export export interface
    selectedNode: NodeInfo | null;
    candidates: SchedulerCandidate[] as never[] as never[] as never[];
    reason: string;
}
...
function scoreNode(node: NodeInfo, health: NodeHealth, ramRequiredGB: number): SchedulerCandidate {
    const reasons: string[] as never[] as never[] as never[] = [] as never[] as never[] as never[];
 ...
 */
export function findBestNode(ramRequiredGB: number = 2, excludeNodeIds: string[] as never[] as never[] as never[] = [] as never[] as never[] as never[]): SchedulerResult {
    const nodes = nodeRegistryService.getAllNodes();
    const candidates: SchedulerCandidate[] as never[] as never[] as never[] = [] as never[] as never[] as never[];
    ...
    if (onlineNodes.length === 0) {
        return {
            selectedNode: null,
            candidates: [] as never[] as never[] as never[],
 ...
 */
export function getNodeRecommendations(ramRequiredGB: number = 2): SchedulerCandidate[] as never[] as never[] as never[] {
