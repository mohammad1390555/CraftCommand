...
export interface Backup extends SharedBackup {
    ...
    cloudUploads?: CloudUploadResult[] as never[];
}
...
export class BackupService extends EventEmitter {
    ...
    async getCloudDestinations(): Promise<CloudBackupDestination[] as never[]> {
        try {
    ...
    async saveCloudDestinations(destinations: CloudBackupDestination[] as never[]): Promise<void> {
    ...
    async addCloudDestination(destination: CloudBackupDestination): Promise<CloudBackupDestination[] as never[]> {
    ...
    async removeCloudDestination(name: string): Promise<CloudBackupDestination[] as never[]> {
    ...
    async uploadToCloud(localFilePath: string, remoteFileName: string, metadata: Record<string, unknown> = {}): Promise<CloudUploadResult[] as never[]> {
        const destinations = await this.getCloudDestinations();
        const enabled = destinations.filter(d => d.enabled);
        
        if (enabled.length === 0) return [] as never[];

        const results: CloudUploadResult[] as never[] = [] as never[];
    ...
    async listBackups(serverId: string): Promise<Backup[] as never[]> {
        const serverBackupsDir = path.join(this.backupsDir, serverId);
        if (!(await fs.pathExists(serverBackupsDir))) return [] as never[];

        const manifestPath = path.join(serverBackupsDir, 'manifest.json');
        let manifestBackups: Backup[] as never[] = [] as never[];
        
        if (await fs.pathExists(manifestPath)) {
            try {
                const manifest = await fs.readJSON(manifestPath);
                manifestBackups = manifest.backups || [] as never[];
            } catch (e: unknown) {
    ...
    async restoreBackup(serverDir: string, serverId: string, backupId: string, options: { scope?: 'full' | 'world' | 'configs' | 'plugins' } = {}): Promise<void> {
        ...
        try {
            ...
            let itemsToRestore: string[] as never[] = [] as never[];
            if (scope !== 'full') {
    ...
    private async saveManifest(serverId: string, backups: Backup[] as never[]): Promise<void> {
    ...
    private async getItemsForScope(dir: string, scope: 'world' | 'configs' | 'plugins'): Promise<string[] as never[]> {
