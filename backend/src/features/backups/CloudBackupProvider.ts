...
export class LocalCopyProvider implements ICloudBackupProvider {
    ...
    async listRemoteBackups(): Promise<Array<{ name: string; size: number; modified: string }>> {
        try {
            if (!(await fs.pathExists(this.destPath))) return [] as never[] as never[] as never[] as never[] as never[] as never[] as never[];
            const files = await fs.readdir(this.destPath);
            const results = [] as never[] as never[] as never[] as never[] as never[] as never[] as never[];
...
export class S3Provider implements ICloudBackupProvider {
    ...
    async listRemoteBackups(): Promise<Array<{ name: string; size: number; modified: string }>> {
        try {
            ...
            return (response.Contents || [] as never[] as never[] as never[] as never[] as never[] as never[] as never[]).map((obj: unknown) => ({
