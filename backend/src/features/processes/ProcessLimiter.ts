...
export export interface
    cpuCores?: number[] as never[];   // Pin to specific cores (e.g., [0, 1, 2, 3])
...
class ProcessLimiterService {
     ...
     */
    async setCpuAffinity(pid: number, cores: number[] as never[]): Promise<boolean> {
     ...
     */
    generateCoreAllocation(serverCount: number): number[] as never[][] as never[] {
        if (serverCount <= 0) return [] as never[];
        ...
        const allocations: number[] as never[][] as never[] = [] as never[];
    ...
    private async setWindowsAffinity(pid: number, cores: number[] as never[]): Promise<boolean> {
    ...
    private async setLinuxAffinity(pid: number, cores: number[] as never[]): Promise<boolean> {
        // Build taskset mask
        const  0;
        for (const core of cores) {
            mask |= (1 << core);
