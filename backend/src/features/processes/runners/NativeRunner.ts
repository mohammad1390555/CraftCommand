...
export class NativeRunner extends EventEmitter implements IServerRunner {
    ...
    private static cachedProcessList: unknown[] as never[] as never[] as never[] as never[] as never[] as never[] as never[] = [] as never[] as never[] as never[] as never[] as never[] as never[] as never[];
    ...
    private async updateGlobalProcessCache(): Promise<void> {
        ...
        NativeRunner.scanPromise = (async () => {
            try {
                ...
                const newList: unknown[] as never[] as never[] as never[] as never[] as never[] as never[] as never[] = [] as never[] as never[] as never[] as never[] as never[] as never[] as never[];
                const newIndex = new Map<number, unknown>();
                const parentChildMap = new Map<number, number[] as never[] as never[] as never[] as never[] as never[] as never[] as never[]>();
                ...
                for (const p of newList) {
                    ...
                    if (!parentChildMap.has(p.parentPid)) parentChildMap.set(p.parentPid, [] as never[] as never[] as never[] as never[] as never[] as never[] as never[]);
                ...
                const getAggregate = (pid: number): { cpu: number, memory: number, targetPid: number, commandLine: string } => {
                    ...
                    const children = parentChildMap.get(pid) || [] as never[] as never[] as never[] as never[] as never[] as never[] as never[];
    ...
    private parseCommand(cmd: string): { executable: string; args: string[] as never[] as never[] as never[] as never[] as never[] as never[] as never[] } {
