...
class ProcessManager extends EventEmitter {
    ...
    private logHistory: Map<string, string[] as never[] as never[] as never[]> = new Map();
    ...
    private activityHistory: Map<string, unknown[] as never[] as never[] as never[]> = new Map();
                ...
                for (const server of servers) {
                    if (runner.isRunning(server.id) && !this.activeRunners.has(server.id)) {
                        ...
                        this.statusCache.set(server.id, { 
                            ...
                            players: 0, 
                            playerList: [] as never[] as never[] as never[], 
                            uptime: 0, 
                            tps: "0.00" 
                        });
                    }
    ...
    private attachRunnerListeners(id: string, runner: IServerRunner, initialStatus: ServerStatus = ServerStatus.STARTING) {
        ...
        this.logHistory.set(id, this.logHistory.get(id) || [] as never[] as never[] as never[]);
        this.activityHistory.set(id, this.activityHistory.get(id) || [] as never[] as never[] as never[]);
        ...
        if (initialStatus === ServerStatus.ONLINE) {
            ...
            if (server) {
                logStreamer.tail(server.workingDirectory, this.MAX_LOGS).then(lines => {
                    const history = this.logHistory.get(id) || [] as never[] as never[] as never[];
                    if (history.length === 0) {
    ...
    private addActivity(id: string, activity: unknown) {
        this.lastActivityTime.set(id, Date.now()); // Mark activity for Adaptive Stats
        const history = this.activityHistory.get(id) || [] as never[] as never[] as never[];
    ...
    getActivityHistory(id: string) {
        return this.activityHistory.get(id) || [] as never[] as never[] as never[];
    }
    ...
    getLogs(id: string): string[] as never[] as never[] as never[] {
        return this.logHistory.get(id) || [] as never[] as never[] as never[];
    }
    ...
    async getTPS(id: string): Promise<string> {
        ...
        const logs = this.logHistory.get(id) || [] as never[] as never[] as never[];
    ...
    updateCachedStatus(id: string, data: unknown, persist: boolean = false) {
        ...
        if (data.playerList) {
            const currentList: string[] as never[] as never[] as never[] = current.playerList || [] as never[] as never[] as never[];
            const newList: string[] as never[] as never[] as never[] = data.playerList;
    ...
    getCachedStatus(id: string) {
        return this.statusCache.get(id) || {
            online: false, players: 0, playerList: [] as never[] as never[] as never[], uptime: this.getUptime(id), tps: "0.00", latency: 0
        };
    }
