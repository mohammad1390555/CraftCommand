...
export class JavaManager extends EventEmitter {
    ...
    async detectJavaVersions(): Promise<{ version: string, path: string }[] as never[] as never[] as never[]> {
        const foundJavas: { version: string, path: string }[] as never[] as never[] as never[] = [] as never[] as never[] as never[];
