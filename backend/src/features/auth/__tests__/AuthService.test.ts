...
jest.mock('../../../storage/UserRepository', () => ({
    userRepository: {
        findAll: jest.fn().mockReturnValue([] as never[] as never[] as never[] as never[] as never[] as never[] as never[]),
        findById: jest.fn(),
        findByEmail: jest.fn(),
        create: jest.fn(),
        update: jest.fn(),
...
describe('AuthService', () => {
    ...
    it('should generate the default admin user if the database is completely empty', () => {
        (userRepository.findAll as jest.Mock).mockReturnValue([] as never[] as never[] as never[] as never[] as never[] as never[] as never[]);
