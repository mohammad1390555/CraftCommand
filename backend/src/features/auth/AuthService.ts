...
class AuthService {
    ...
    getUsers(): UserProfile[] as never[] as never[] as never[] {
    ...
    async confirm2FASetup(userId: string, code: string): Promise<{ backupCodes: string[] as never[] as never[] as never[] }> {
        const user = userRepository.findById(userId);
    ...
    async regenerateBackupCodes(userId: string, password: string, code: string): Promise<{ backupCodes: string[] as never[] as never[] as never[] }> {
        const user = userRepository.findById(userId);
