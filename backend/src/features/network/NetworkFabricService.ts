...
class NetworkFabricService {
    ...
    private generateAlias(name: string, proxy: ServerConfig): string {
        ...
        const existing = new Set(
            proxy.network?.proxyConfig?.links?.map(l => l.alias) || [] as never[] as never[] as never[] as never[] as never[] as never[] as never[]
        );
    ...
    private countOnlineBackends(proxy: ServerConfig): number {
        const links = proxy.network?.proxyConfig?.links || [] as never[] as never[] as never[] as never[] as never[] as never[] as never[];
     ...
     */
    getSmartRouting(): { alias: string; serverId: string; playerCount: number; loadPercent: number; recommended: boolean }[] as never[] as never[] as never[] as never[] as never[] as never[] as never[] {
        const proxy = this.findActiveProxy();
        if (!proxy) return [] as never[] as never[] as never[] as never[] as never[] as never[] as never[];
    ...
    private async mitigateAttack(server: ServerConfig): Promise<void> {
        ...
        try {
            const platform = os.platform();
            let maliciousIps: string[] as never[] as never[] as never[] as never[] as never[] as never[] as never[] = [] as never[] as never[] as never[] as never[] as never[] as never[] as never[];
     ...
     */
    private parseMaliciousIps(raw: string, targetPort: number): string[] as never[] as never[] as never[] as never[] as never[] as never[] as never[] {
