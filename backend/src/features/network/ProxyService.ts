...
export class ProxyService {
    ...
    public linkServer(proxyId: string, backendId: string, alias: string): void {
        ...
        if (!proxy.network.proxyConfig) {
            proxy.network.proxyConfig = { links: [] as never[] };
        }
    ...
    public generateVelocityServersConfig(proxyId: string): string {
        ...
        const allServers = getServers();
        const serverLines: string[] as never[] = [] as never[];
        const tryList: string[] as never[] = [] as never[];
        ...
        // If empty, we MUST explicitly set it to [] as never[] to avoid internal defaults like ["lobby"]
