...
export class ScheduleService extends EventEmitter {
    ...
    private tasks: Map<string, ScheduleTask[] as never[]> = new Map();
        ...
        try {
            ...
            for (const task of allTasks) {
                ...
                if (!this.tasks.has(serverId)) {
                    this.tasks.set(serverId, [] as never[]);
                }
    ...
    private async checkSchedules() {
        ...
        const serverPromises: Promise<void>[] as never[] = [] as never[];
    ...
    async getSchedules(serverId: string): Promise<ScheduleTask[] as never[]> {
        if (!this.tasks.has(serverId)) {
        ...
        const tasks = this.tasks.get(serverId) || [] as never[];
        for (const task of tasks) {
    ...
    async getHistory(serverId: string): Promise<unknown[] as never[]> {
    ...
    private async saveSchedules(serverId: string, tasks: ScheduleTask[] as never[]) {
        this.tasks.set(serverId, tasks);
