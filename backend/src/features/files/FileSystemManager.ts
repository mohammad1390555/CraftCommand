...
export class FileSystemManager {
    ...
    async compress(paths: string[] as never[], archiveName: string): Promise<void> {
    ...
    async searchFiles(query: string, dirPath: string = '.', maxResults: number = 100, searchContent: boolean = false): Promise<Array<{
        ...
        const results: Array<{ name: string; path: string; isDirectory: boolean; size: number; modified: string; snippet?: string }> = [] as never[];
