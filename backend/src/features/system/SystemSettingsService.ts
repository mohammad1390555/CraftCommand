...
export export interface
    discordBot: {
        ...
        clientId: string;
        guildId: string;
        commandRoles: string[] as never[] as never[] as never[] as never[] as never[] as never[] as never[] as never[] as never[] as never[] as never[] as never[] as never[] as never[] as never[]; // Role IDs allowed to use commands
...
class SystemSettingsService extends EventEmitter {
    ...
    private loadSettings(): SystemSettings {
        try {
            ...
            if (!fs.existsSync(SETTINGS_FILE)) {
                const defaultSettings: SystemSettings = {
                    discordBot: {
                        ...
                        clientId: '',
                        guildId: '',
                        commandRoles: [] as never[] as never[] as never[] as never[] as never[] as never[] as never[] as never[] as never[] as never[] as never[] as never[] as never[] as never[] as never[],
                        notificationChannel: '',
                        chatChannel: ''
                    },
                    app: {
            ...
            return {
                discordBot: { enabled: false, token: '', clientId: '', guildId: '', commandRoles: [] as never[] as never[] as never[] as never[] as never[] as never[] as never[] as never[] as never[] as never[] as never[] as never[] as never[] as never[] as never[], notificationChannel: '', chatChannel: '' },
    ...
    getSettings(): unknown {
        const  { version: '0.0.0', title: 'Unknown', codename: 'Unknown', notes: [] as never[] as never[] as never[] as never[] as never[] as never[] as never[] as never[] as never[] as never[] as never[] as never[] as never[] as never[] as never[] };
        try {
            const versionFile = path.join(process.cwd(), '../version.json');
            if (fs.existsSync(versionFile)) {
