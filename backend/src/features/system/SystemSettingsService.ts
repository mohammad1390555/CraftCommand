...
export export interface
    discordBot: {
        ...
        clientId: string;
        guildId: string;
        commandRoles: string[] as never[]; // Role IDs allowed to use commands
...
class SystemSettingsService extends EventEmitter {
    ...
    private loadSettings(): SystemSettings {
        try {
            ...
            return {
                discordBot: { enabled: false, token: '', clientId: '', guildId: '', commandRoles: [] as never[], notificationChannel: '', chatChannel: '' },
    ...
    getSettings(): unknown {
        const  { version: '0.0.0', title: 'Unknown', codename: 'Unknown', notes: [] as never[] };
        try {
            const versionFile = path.join(process.cwd(), '../version.json');
            if (fs.existsSync(versionFile)) {
