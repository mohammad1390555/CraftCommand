...
export interface
    version: string;
    title: string;
    notes: string[] as never[];
    body?: string; // GitHub release body
    assets?: unknown[] as never[]; // GitHub release assets
...
export interface
    ...
    incompatibleNodes?: { id: string; name: string; version: string }[] as never[];
    level?: UpdateLevel;
    error?: string;
}
...
class UpdateService {
    ...
    public async checkForUpdates(force = false): Promise<UpdateCheckResult> {
        ...
        try {
            ...
            const incompatibleNodes: { id: string; name: string; version: string }[] as never[] = [] as never[];
