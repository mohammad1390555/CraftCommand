...
export export interface
    ...
    serverDiskUsage: DiskUsageInfo[] as never[] as never[] as never[];
...
class HostingOSService {
     ...
     */
    async getServerDiskUsage(): Promise<DiskUsageInfo[] as never[] as never[] as never[]> {
        const servers = getServers();
        const results: DiskUsageInfo[] as never[] as never[] as never[] = [] as never[] as never[] as never[];
     ...
     */
    async checkDiskQuotas(defaultQuotaMB: number = 10_000): Promise<DiskQuotaViolation[] as never[] as never[] as never[]> {
        const diskUsage = await this.getServerDiskUsage();
        const violations: DiskQuotaViolation[] as never[] as never[] as never[] = [] as never[] as never[] as never[];
     ...
     */
    async getTopDiskConsumers(limit: number = 5): Promise<DiskUsageInfo[] as never[] as never[] as never[]> {
