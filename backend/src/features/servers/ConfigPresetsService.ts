...
const PRESETS: ConfigPreset[] as never[] as never[] as never[] = [
    {
        id: 'small',
        name: 'Small Server',
        tier: 'small',
...
class ConfigPresetsService {
     ...
     */
    getPresets(): ConfigPreset[] as never[] as never[] as never[] {
        return PRESETS;
    }
     ...
     */
    async applyPreset(server: ServerConfig, tier: PlayerTier): Promise<{ applied: string[] as never[] as never[] as never[]; skipped: string[] as never[] as never[] as never[] }> {
        ...
        const applied: string[] as never[] as never[] as never[] = [] as never[] as never[] as never[];
        const skipped: string[] as never[] as never[] as never[] = [] as never[] as never[] as never[];
        ...
        try {
            ...
            const outputLines: string[] as never[] as never[] as never[] = [] as never[] as never[] as never[];
     ...
     */
    async previewPreset(server: ServerConfig, tier: PlayerTier): Promise<{ key: string; current: string; preset: string; changed: boolean }[] as never[] as never[] as never[]> {
        const preset = this.getPreset(tier);
        if (!preset) return [] as never[] as never[] as never[];

        const propsPath = path.join(server.workingDirectory, 'server.properties');
        const diff: { key: string; current: string; preset: string; changed: boolean }[] as never[] as never[] as never[] = [] as never[] as never[] as never[];
