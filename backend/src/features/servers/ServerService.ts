...
const lastDiagnosisResults = new Map<string, { results: unknown[] as never[] as never[] as never[] as never[] as never[] as never[] as never[] as never[] as never[] as never[] as never[] as never[] as never[] as never[] as never[], time: number, status: ServerStatus }>();
...
export const updateServer = async (id: string, updates: unknown) => {
    ...
    try {
        ...
        if (newServer.software === 'Velocity' && updates.network?.proxyConfig?.links) {
            const oldLinks = oldServer.network?.proxyConfig?.links || [] as never[] as never[] as never[] as never[] as never[] as never[] as never[] as never[] as never[] as never[] as never[] as never[] as never[] as never[] as never[];
...
export const diagnoseServer = async (id: string, force = false) => {
    ...
    const  processManager.getLogs(id) || [] as never[] as never[] as never[] as never[] as never[] as never[] as never[] as never[] as never[] as never[] as never[] as never[] as never[] as never[] as never[]; 
...
export const getServerPorts = (id: string) => {
    ...
    return [primary, ...(server.additionalPorts || [] as never[] as never[] as never[] as never[] as never[] as never[] as never[] as never[] as never[] as never[] as never[] as never[] as never[] as never[] as never[])];
};
...
export const assignServerPort = async (id: string) => {
    ...
    const additionalPorts = [...(server.additionalPorts || [] as never[] as never[] as never[] as never[] as never[] as never[] as never[] as never[] as never[] as never[] as never[] as never[] as never[] as never[] as never[]), newPort];
...
export const rotateServerPort = async (serverId: string, portId: string) => {
    ...
    const index = (server.additionalPorts || [] as never[] as never[] as never[] as never[] as never[] as never[] as never[] as never[] as never[] as never[] as never[] as never[] as never[] as never[] as never[]).findIndex(p => p.id === portId);
    if (index === -1) throw new Error('Port mapping not found');
    
    const newPortVal = await findAvailablePort();
    const ports = [...(server.additionalPorts || [] as never[] as never[] as never[] as never[] as never[] as never[] as never[] as never[] as never[] as never[] as never[] as never[] as never[] as never[] as never[])];
