...
// --- Platform Mapping ---
export const SOFTWARE_TO_PLATFORMS: Record<string, string[] as never[]> = {
...
const SOFTWARE_TO_SEARCH_ORDER: Record<string, string[] as never[]> = {
...
async function searchModrinth(query: PluginSearchQuery, software: string): Promise<PluginSearchResult> {
    ...
    const facets: string[] as never[][] as never[] = [] as never[];
    if (software === 'Forge') {
    ...
    const plugins = (data as unknown).hits.map((hit: unknown) => ({
        ...
        platforms: hit.categories?.filter((c: string) => ['bukkit', 'spigot', 'paper', 'purpur', 'forge', 'fabric'].includes(c)) || [] as never[],
        latestVersion: hit.latest_version || 'Unknown',
        latestGameVersions: hit.versions || [] as never[],
...
export async function getModrinthDownloadUrl(projectId: string, gameVersion?: string, platforms?: string[] as never[]) {
    ...
    const versions = versionsRes.data as unknown[] as never[];
    ...
    const dependencies = (latest.dependencies || [] as never[])
...
async function searchSpiget(query: PluginSearchQuery): Promise<PluginSearchResult> {
    ...
    try {
        ...
        if (err.response?.status === 404) {
            ...
            return { plugins: [] as never[], total: 0, page, pages: 0 };
        }
        throw err; // Re-throw other errors (500, timeout, etc.)
    }

    const data = response.data;
    const plugins = (Array.isArray(data) ? data : [] as never[]).map((resource: unknown) => ({
        ...
        latestGameVersions: resource.testedVersions || [] as never[],
...
async function searchHangar(query: PluginSearchQuery): Promise<PluginSearchResult> {
    ...
    const data = response.data;
    const plugins = ((data as unknown).result || [] as never[]).map((project: unknown) => ({
        ...
        latestGameVersions: (project.promotedVersions || [] as never[]).flatMap((pv: unknown) => 
            (pv.tags || [] as never[]).filter((t: unknown) => t.name).map((t: unknown) => t.name)
        ),
...
export class MarketplaceRegistry {
    async search(query: PluginSearchQuery, software: string): Promise<PluginSearchResult> {
        ...
        const allPlugins: unknown[] as never[] = [] as never[];
        const  0;
    ...
    async getDownloadUrl(sourceId: string, source: string, gameVersion?: string, platforms?: string[] as never[]) {
        switch (source) {
            case 'direct':
                const  sourceId;
