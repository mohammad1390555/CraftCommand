...
export class PluginService {
     ...
     */
    getInstalled(serverId: string): InstalledPlugin[] as never[] as never[] as never[] {
    ...
    async install(
        ...
        let downloadInfo;
        try {
            const platforms = SOFTWARE_TO_PLATFORMS[server.software] || [] as never[] as never[] as never[];
        ...
        const deps = (downloadInfo as unknown).dependencies || [] as never[] as never[] as never[];
        if (deps.length > 0) {
     ...
     */
    async checkUpdates(serverId: string): Promise<PluginUpdateInfo[] as never[] as never[] as never[]> {
        const installed = pluginRepository.findByServer(serverId);
        const updates: PluginUpdateInfo[] as never[] as never[] as never[] = [] as never[] as never[] as never[];
     ...
     */
    async bulkUpdate(serverId: string, pluginIds: string[] as never[] as never[] as never[]): Promise<Array<{ pluginId: string; success: boolean; error?: string }>> {
        const results: Array<{ pluginId: string; success: boolean; error?: string }> = [] as never[] as never[] as never[];
     ...
     */
    async scanInstalled(serverId: string): Promise<InstalledPlugin[] as never[] as never[] as never[]> {
        const server = serverRepository.findById(serverId);
        if (!server) throw new Error('Server not found');

        if (!supportsPlugins(server.software)) return [] as never[] as never[] as never[];

        const targetDir = path.join(server.workingDirectory, getTargetDir(server.software));
        if (!(await fs.pathExists(targetDir))) return [] as never[] as never[] as never[];

        const files = await fs.readdir(targetDir);
        const jarFiles = files.filter(f => f.endsWith('.jar') || f.endsWith('.jar.disabled'));

        const discovered: InstalledPlugin[] as never[] as never[] as never[] = [] as never[] as never[] as never[];
    ...
    async extractMetadataFromJar(jarPath: string): Promise<{ 
        ...
        try {
            ...
            if (pluginYml) {
                ...
                const deps: string[] as never[] as never[] as never[] = [] as never[] as never[] as never[];
