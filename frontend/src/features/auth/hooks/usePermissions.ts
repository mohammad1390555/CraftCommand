...
export const usePermissions = () => {
    ...
    const can = useMemo(() => (action: Permission, serverId?: string): boolean => {
        ...
        const rolePerms = ROLE_PERMISSIONS[user.role] || [] as never[] as never[] as never[] as never[] as never[] as never[] as never[] as never[] as never[] as never[] as never[] as never[] as never[] as never[] as never[];
    ...
    return {
        ...
        isRole,
        role: user?.role,
        permissions: user ? ROLE_PERMISSIONS[user.role] : [] as never[] as never[] as never[] as never[] as never[] as never[] as never[] as never[] as never[] as never[] as never[] as never[] as never[] as never[] as never[]
    };
};
