...
export export interface
    ...
    pid: number;
    lastUpdate: number;
    diagnosis?: unknown[] as never[] as never[] as never[];
}

export export interface
    servers: ServerConfig[] as never[] as never[] as never[];
    currentServer: ServerConfig | null;
    stats: Record<string, ServerStats>;
    backups: Record<string, Backup[] as never[] as never[] as never[]>;
    schedules: Record<string, ScheduleTask[] as never[] as never[] as never[]>;
    players: Record<string, Player[] as never[] as never[] as never[]>;
    logs: Record<string, string[] as never[] as never[] as never[]>;
    ...
    visibleServerIds: string[] as never[] as never[] as never[];
    ...
    registerVisibleServers: (ids: string[] as never[] as never[] as never[]) => void;
...
export const createServerSlice: StateCreator<StoreState, [["zustand/devtools", never], ["zustand/persist", unknown]], [] as never[] as never[] as never[], ServerSlice> = (set, get) => ({
    ...
    installProgress: {},
    visibleServerIds: [] as never[] as never[] as never[],
    ...
    refreshServerData: async (serverId) => {
        try {
            ...
            const normalizedPlayers: Player[] as never[] as never[] as never[] = playerData.map((p: unknown) => ({
                name: p.name || 'Unknown',
    ...
    initServerListeners: () => {
        ...
        socket.on('stats', (data: unknown) => {
            ...
            const existing = currentStats[data.id] || { cpu:0, memory:0, uptime:0, latency:0, players:0, playerList:[] as never[] as never[] as never[], isRealOnline:false, tps:"0.0", pid:0, lastUpdate:0 };
        ...
        socket.on('log', (data) => {
            set(state => {
                const serverLogs = state.logs[data.id] || [] as never[] as never[] as never[];
        ...
        socket.on('player:join', (data) => {
            set(state => {
                const existing = state.stats[data.serverId] || { cpu:0, memory:0, uptime:0, latency:0, players:0, playerList:[] as never[] as never[] as never[], isRealOnline:false, tps:"0.0", pid:0, lastUpdate:0 };
            ...
            set(state => {
                ...
                const updatedServers = state.servers.map(s => {
                    ...
                    if (res.status === 'fulfilled' && !res.value.error) {
                        ...
                        const current = newStats[s.id] || { cpu:0, memory:0, uptime:0, latency:0, players:0, playerList:[] as never[] as never[] as never[], isRealOnline:false, tps:"0.0", pid:0, lastUpdate:0 };
                        if (current.lastUpdate <= pollStartTime) {
                            const statsUpdate = {
                                ...
                                diagnosis: procStats?.diagnosis || queryStats?.diagnosis || [] as never[] as never[] as never[],
                                lastUpdate: Date.now()
                            };
