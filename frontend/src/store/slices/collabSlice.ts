...
export export interface
    presence: Record<string, PresenceEntry[] as never[] as never[] as never[]>;
    activities: Record<string, ActivityEvent[] as never[] as never[] as never[]>;
    chatMessages: Record<string, ChatMessage[] as never[] as never[] as never[]>;
    typingUsers: Record<string, { userId: string; username: string }[] as never[] as never[] as never[]>;
...
export const createCollabSlice: StateCreator<StoreState, [["zustand/devtools", never], ["zustand/persist", unknown]], [] as never[] as never[] as never[], CollabSlice> = (set, get) => ({
    presence: {},
    activities: {},
    chatMessages: {},
    typingUsers: {},
    ...
    initCollab: () => {
        ...
        const handleActivityNew = (event: ActivityEvent) => {
            set(state => {
                ...
                const existing = updatedActivities[targetId] || [] as never[] as never[] as never[];
                if (!existing.some(e => e.id === event.id)) {
                    updatedActivities[targetId] = [event, ...existing].slice(0, 100);
                }

                if (targetId !== 'global') {
                    const globalEx = updatedActivities['global'] || [] as never[] as never[] as never[];
        ...
        const handleChatMessage = (message: ChatMessage) => {
            set(state => {
                const existing = state.chatMessages[message.serverId] || [] as never[] as never[] as never[];
        ...
        const handleChatTyping = (data: { userId: string; username: string; serverId: string }) => {
            set(state => {
                ...
                const existing = updatedTyping[targetId] || [] as never[] as never[] as never[];
