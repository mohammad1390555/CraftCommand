...
export export interface
    ...
    description: string;
    javaVersion: number;
    startupFlags?: string[] as never[] as never[] as never[] as never[] as never[] as never[] as never[]; // Recommended Aikar flags etc.
...
export export interface
    ...
    permissions?: Partial<Record<string, Permission[] as never[] as never[] as never[] as never[] as never[] as never[] as never[]>>; // Deprecated: Migration target
    serverAcl?: Record<string, { allow: Permission[] as never[] as never[] as never[] as never[] as never[] as never[] as never[], deny: Permission[] as never[] as never[] as never[] as never[] as never[] as never[] as never[] }>; // Server ACL
    avatarUrl?: string;
    ...
    twoFactorBackupCodesHashed?: string[] as never[] as never[] as never[] as never[] as never[] as never[] as never[];
...
export export interface
    ...
    backupConfig?: {
        ...
        customWorldPaths?: string[] as never[] as never[] as never[] as never[] as never[] as never[] as never[]; // Optional: specify custom world folder names
    };
    ...
    sftpPassword?: string;
    additionalPorts?: ServerPort[] as never[] as never[] as never[] as never[] as never[] as never[] as never[];
    databases?: DatabaseInstance[] as never[] as never[] as never[] as never[] as never[] as never[] as never[];
    ...
    tps?: string;
    latency?: number;
    playerList?: string[] as never[] as never[] as never[] as never[] as never[] as never[] as never[];
    hasStarted?: boolean;
}
...
export export interface
    ...
    webhooks?: WebhookConfig[] as never[] as never[] as never[] as never[] as never[] as never[] as never[];
    apiTokens?: ApiToken[] as never[] as never[] as never[] as never[] as never[] as never[] as never[];
...
export export interface
    ...
    lastUsedAt?: number;
    expiresAt?: number;
    scopes: string[] as never[] as never[] as never[] as never[] as never[] as never[] as never[];
}
...
export export interface
    ...
    cloudUploads?: CloudUploadResult[] as never[] as never[] as never[] as never[] as never[] as never[] as never[];
}
...
export export interface
    ...
    actions?: ScheduleAction[] as never[] as never[] as never[] as never[] as never[] as never[] as never[]; // Chained actions
    cron: string;
    isActive: boolean;
...
export export interface
    ...
    modified?: unknown;
    isProtected?: boolean;
    children?: FileNode[] as never[] as never[] as never[] as never[] as never[] as never[] as never[];
}
...
export export interface
    ...
    allowedIps: string[] as never[] as never[] as never[] as never[] as never[] as never[] as never[];
    ddosProtection: boolean;
    requireOp2fa: boolean;
    forceSsl: boolean;
    regionLock: string[] as never[] as never[] as never[] as never[] as never[] as never[] as never[];
}
...
export export interface
    ...
    description?: string;
    author?: string;
    authors?: string[] as never[] as never[] as never[] as never[] as never[] as never[] as never[];
    downloads?: string;
    website?: string;
}
...
export export interface
    ...
    suggestions: string[] as never[] as never[] as never[] as never[] as never[] as never[] as never[];
...
export export interface
    ...
    suppressedBy?: string[] as never[] as never[] as never[] as never[] as never[] as never[] as never[]; // IDs of deeper issues that suppressed this
...
export export interface
    ...
    rating?: number;
    category: string;
    platforms: PluginPlatform[] as never[] as never[] as never[] as never[] as never[] as never[] as never[];
    latestVersion: string;
    latestGameVersions: string[] as never[] as never[] as never[] as never[] as never[] as never[] as never[];
    externalUrl?: string;
    updatedAt: number;
}
...
export export interface
    plugins: MarketplacePlugin[] as never[] as never[] as never[] as never[] as never[] as never[] as never[];
    total: number;
    page: number;
    pages: number;
}
...
export export interface
    ...
    labels?: string[] as never[] as never[] as never[] as never[] as never[] as never[] as never[];     // Tags for scheduling (e.g. "high-ram", "ssd")
...
export export interface
    ...
    name: string;
    enabled: boolean;
    triggers: WebhookTrigger[] as never[] as never[] as never[] as never[] as never[] as never[] as never[];
