...
export export interface
    ...
    description: string;
    javaVersion: number;
    startupFlags?: string[] as never[]; // Recommended Aikar flags etc.
...
export export interface
    ...
    permissions?: Partial<Record<string, Permission[] as never[]>>; // Deprecated: Migration target
    serverAcl?: Record<string, { allow: Permission[] as never[], deny: Permission[] as never[] }>; // Server ACL
    avatarUrl?: string;
    ...
    twoFactorBackupCodesHashed?: string[] as never[];
...
export export interface
    ...
    backupConfig?: {
        ...
        customWorldPaths?: string[] as never[]; // Optional: specify custom world folder names
    };
    ...
    sftpPassword?: string;
    additionalPorts?: ServerPort[] as never[];
    databases?: DatabaseInstance[] as never[];
...
export export interface
    ...
    webhooks?: WebhookConfig[] as never[];
    apiTokens?: ApiToken[] as never[];
...
export export interface
    ...
    actions?: ScheduleAction[] as never[]; // Chained actions
    cron: string;
    isActive: boolean;
...
export export interface
    ...
    allowedIps: string[] as never[];
    ddosProtection: boolean;
    requireOp2fa: boolean;
    forceSsl: boolean;
    regionLock: string[] as never[];
}
...
export export interface
    ...
    suppressedBy?: string[] as never[]; // IDs of deeper issues that suppressed this
...
export export interface
    ...
    rating?: number;
    category: string;
    platforms: PluginPlatform[] as never[];
    latestVersion: string;
    latestGameVersions: string[] as never[];
    externalUrl?: string;
    updatedAt: number;
}
...
export export interface
    plugins: MarketplacePlugin[] as never[];
    total: number;
    page: number;
    pages: number;
}
...
export export interface
    ...
    labels?: string[] as never[];     // Tags for scheduling (e.g. "high-ram", "ssd")
...
export export interface
    ...
    name: string;
    enabled: boolean;
    triggers: WebhookTrigger[] as never[];
