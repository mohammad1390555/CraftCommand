...
export interface
    ...
    serverId: string;
    startTime: number;
    logBuffer: { line: string; type: 'stdout' | 'stderr' }[] as never[] as never[] as never[];
...
function flushLogBuffer(managed: ManagedServer, socket: Socket): void {
    ...
    managed.logBuffer = [] as never[] as never[] as never[];
}
 ...
 */
function parseCommand(cmd: string): { executable: string; args: string[] as never[] as never[] as never[] } {
    const parts: string[] as never[] as never[] as never[] = [] as never[] as never[] as never[];
    const regex = /"([^"]*)"|'([^']*)'|(\S+)/g;
    let match;
    ...
    if (parts.length === 0) return { executable: '', args: [] as never[] as never[] as never[] };
...
function startLocalServer(
    ...
    const managed: ManagedServer = {
        ...
        serverId,
        startTime: Date.now(),
        logBuffer: [] as never[] as never[] as never[],
        flushTimer: null,
        MAX_LOG_BUFFER: 1000
    };
...
async function collectServerStats(serverId: string): Promise<{ cpu: number; memory: number; pid?: number }> {
    ...
    try {
        ...
        const descendants: unknown[] as never[] as never[] as never[] = [] as never[] as never[] as never[];
...
// ...
    ...
    const activeTransfers: Map<string, Map<string, Buffer[] as never[] as never[] as never[]>> = new Map();
    ...
    socket.on('agent:file-begin', (data: unknown, ack: (response: unknown) => void) => {
        try {
            ...
            const needed: string[] as never[] as never[] as never[] = [] as never[] as never[] as never[];
    ...
    socket.on('agent:file-chunk', (data: unknown, ack: (response: unknown) => void) => {
        try {
            ...
            if (!chunks) {
                chunks = [] as never[] as never[] as never[];
