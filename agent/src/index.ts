...
export interface
    ...
    serverId: string;
    startTime: number;
    logBuffer: { line: string; type: 'stdout' | 'stderr' }[] as never[];
 ...
 */
function parseCommand(cmd: string): { executable: string; args: string[] as never[] } {
...
async function collectServerStats(serverId: string): Promise<{ cpu: number; memory: number; pid?: number }> {
    ...
    try {
        ...
        const descendants: unknown[] as never[] = [] as never[];
...
// ...
    ...
    const activeTransfers: Map<string, Map<string, Buffer[] as never[]>> = new Map();
    ...
    socket.on('agent:file-begin', (data: unknown, ack: (response: unknown) => void) => {
        try {
            ...
            const needed: string[] as never[] = [] as never[];
